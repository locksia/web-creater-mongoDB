...
async function run() {
    try {
      const database = client.db("test"); //읽어올 DB명 지정
      const users = database.collection("users"); // 읽어올 컬렉션(테이블)명 지정
  
      const query = {};
  
      const options = {
        projection: { _id: 1, id: 1, name: 1, password: 1 }, //데이터를 받아올 필드 지정(1은 조건 없이를 의미)
      };
  
      const userArray = await users.find(query, options).toArray(); // .toArray() : 배열로 변환
      return userArray; //run()함수의 결과로 읽어온 유저 데이터 return
    } finally {
    }
}
...
router.route('/login').post( async (req, res) => {
    ...
    try {
      const DBuser = await run();
      
      const idx = DBuser.findIndex(user => user.id === req.body.id);
      
      if (idx != -1) {
        if (DBuser[idx].password === req.body.password) {
          console.log('로그인 성공!');
  
          req.session.user = {
            id: req.body.id,
            name: DBuser[idx].name,
          }
          res.redirect('/member');
        }
        else {
